...
<%
    ...
    if (name == null || name.trim().isEmpty() || pno == null || pno.trim().isEmpty()) {
        response.sendRedirect(request.getContextPath() + "/user/find_pw.jsp?msg=0"); // 입력값이 비어있는 경우
        return;
    }
    ...
    String result = userService.findPw(user);

    // 결과값이 null이거나 비어 있는 경우에 대한 처리
    if (result == null || result.isEmpty()) {
        response.sendRedirect(request.getContextPath() + "/user/find_pw.jsp?msg=0"); // 사용자를 찾을 수 없는 경우
        return;
    }

    // 사용자가 존재하는 경우
    request.setAttribute("user_pw", result);
    request.getRequestDispatcher("/user/complete_find_pw.jsp").forward(request, response);
%>
